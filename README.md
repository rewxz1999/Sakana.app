# Sakana 🐟

番剧管理及播放桌面客户端。Electron + Vite + React + TypeScript。

## 功能

- **番剧表**：bangumi 数据源（默认使用**自建反代**，不再自动回退公共镜像；反代不可用时弹窗提示手动切换），周历切换、缓存优先、卡片快捷收藏、评分按需补全
- **仪表盘**：订阅/收藏/工具统计、更新动态、观看与订阅历史、下载任务、时间段分布与**收藏类型分布**圆环、**年度统计横向柱状图**、继续观看
- **订阅**：蜜柑计划 RSS 资源选择（字幕组/集数/大小/日期）、「下载」=订阅+下载 /「订阅」=仅追踪该字幕组、启动自动检测新资源（需确认后下载）、无订阅的已下载资源也显示卡片
- **下载**：内置 aria2（JSON-RPC）或外部 qBittorrent（Web API）；种子先下载再解析；完成后按 `番剧名 字幕组 第XX集.后缀` 重命名；并发数可配置；失败可重试；完成后进度条消失、可直接播放
- **收藏**：按年份分组、垂直年份条拖动定位、全源搜索（回车/按钮触发）、重点关心列表、**「已看完」标签**（观看记录覆盖全部集数自动判定 / 详情页手动标记并显示看完时间）、收藏时后台补全放送年份（修复当年番剧误判未知年份）
- **工具**：导入 Python 脚本（`--sakana-data` / `SAKANA_DATA` 数据接口）、界面内运行、一键导出开发要求文档（MD/TXT）、**内置统计工具 v0.1**（收藏番剧条目管理：序号 202601/个人评分与 bangumi 评分偏差/剧照 1-3/评价 1-3/看完时间）
- **在线播放规则**：Kazumi 风格规则引擎（XPath 栈 xpath-html / API 栈 JSONPath），独立规则配置页，内置 AGE、aafun、TvTFun、7sefun，**支持从 Kazumi 规则仓库导入**（gitcode 镜像优先、GitHub 兜底）；播放流程「规则搜索 → 选集 → 隐藏窗口加载播放页 → 嗅探捕获视频流 → libVLC 直连播放」，用户看不到网页，失败自动回退 iframe
- **播放器**：双播放内核，同一套控制栏，可在「设置 → 播放器设置」切换。**libmpv（默认，推荐）**：项目自带 N-API 原生插件（`native/mpv`）动态加载 `libmpv-2.dll`，在主窗口内创建 WS_CHILD 输出子窗口（含 z 序提升，否则画面会被页面内容盖住而黑屏），渲染层只上报视频区域矩形，因此控件/选集/字幕 UI 与 libVLC 完全一致；**libVLC**（electron-vlc-player）作为兼容性备选，libmpv 不可用时自动回退。**画面比例**支持适应 / 裁剪铺满 / 拉伸铺满三态（控制栏与设置页均可切换）；**全屏时画面铺满整屏，控制栏由独立透明悬浮窗叠加在画面上**（原生窗口无法被网页内容覆盖）。两者均支持 MKV / HEVC / 10bit / 杜比音轨 / 内封与外挂字幕，内建 zh-CN 控制条（音轨/字幕切换、进度、倍速）；内核全部不可用时回退 FFmpeg 转码管线 → HTML5；**控件 5 秒自动隐藏、按钮 pointerdown 加固、截屏移至右侧、详情面板布局同步、可自定义快捷键**（设置 → 播放器快捷键）
- **设置**：5 套主题配色、数据源独立配置页、代理/下载器/路径配置、错误日志（可复制）
- **系统托盘**：关闭时询问「最小化至托盘 / 直接退出 / 取消」；托盘悬浮显示小窗（下载任务可暂停/继续、统计、观看历史）；双击回主界面、右键退出
- **订阅检测**：已存在下载任务的资源不再重复提示；下载完成自动推进订阅的集数与资源日期；无新资源时订阅状态自动复位
- 全部数据本地 JSON 缓存（方案允许 SQLite/JSON，当前 JSON 原子写入），网络请求带超时

## 开发

```bash
npm install          # 首次安装（自动编译 libVLC 原生模块，需 VS Build Tools + Python 3）
npm run dev          # 开发模式（HMR）
npm run typecheck    # 类型检查
npm run build        # 构建
npm run libvlc:fetch # 下载内置 libVLC 运行时（VLC 3.0.x）到 resources/libvlc/（libVLC 内核用）
npm run libmpv:fetch # 下载 libmpv-2.dll（mpv v0.41）到 resources/libmpv/（默认内核，推荐）
npm run mpv:build    # 编译 libmpv 原生插件 → native/mpv/build/Release/sakana_mpv.node（需 VS Build Tools）
npm run ffmpeg:fetch # 下载内置 FFmpeg 到 resources/ffmpeg/（转码回退方案用，可选）
npm run aria2:fetch  # 下载内置 aria2c.exe 到 resources/aria2/（可选）
```

> 注：
> - 本仓库使用 npmmirror 镜像源（`.npmrc`）。
> - **默认播放内核为 libmpv**：需要 `npm run libmpv:fetch`（libmpv-2.dll，约 115MB）与 `npm run mpv:build`（原生插件）；两者缺任一项时自动回退 libVLC。
> - **libVLC 内核（备选）**：运行 `npm run libvlc:fetch` 下载（约 40MB），或安装系统 VLC（自动探测 `C:\Program Files\VideoLAN\VLC`）。
> - `electron-vlc-player` 的原生模块（vlc_binding.node）在 `npm install` 时针对本机 Electron 版本编译，需要 Visual Studio Build Tools（C++ 桌面开发）与 Python 3。
> - 如未安装内置 aria2c，可在「设置 → 下载器配置」中指定 aria2c.exe 路径，或改用 qBittorrent。

## 目录结构

```
src/
├── main/            # Electron 主进程（窗口、存储、IPC、服务）
│   ├── services/    # bangumi / mikan / aria2 / qbit / 下载管理 / 媒体协议 / Python 工具
│   │                # vlc.ts（libVLC）、mpv.ts（libmpv）、playerEngine.ts（内核调度 + 自动回退）
│   └── lib/         # 标题解析、命名规则
├── preload/         # contextBridge 类型化桥接
├── shared/          # 主/渲染共用类型与 IPC 契约
└── renderer/        # React 界面（页面、组件、状态、主题）
native/
└── mpv/             # libmpv 原生插件（N-API，纯 C）：src/addon.cc + binding.gyp
resources/           # 内置运行时（libvlc / libmpv / ffmpeg / aria2，体积大不入库，见 fetch 脚本）
```

## 数据与缓存

- 应用数据：`%APPDATA%/sakana/data/*.json`（设置、收藏、订阅、历史、日志等）
- 网络缓存：`%APPDATA%/sakana/cache/`（番剧表 30 分钟、详情 7 天、图片磁盘缓存）
- 下载目录：设置中配置，自动按番剧名建子文件夹

## 数据源

| 用途 | 地址 | 状态 |
|------|------|------|
| **自建反代（默认，唯一源）** | https://sankana-bangumi.de5.net/api | ✅ 主数据源（`/calendar`、`/v0/subjects/:id`、`POST /v0/search/subjects`）；图片 `…/img` |
| 番组计划(伪) | https://bangumi.vip/ | ⚠️ 备选（清空反代地址后启用，网页镜像 HTML 解析） |
| bangumi 主站 API | https://api.bgm.tv/ | ⚠️ 备选，需代理（设置中配置） |
| 蜜柑计划 | https://mikanani.kas.pub/ | RSS 订阅数据源 |

### 数据源实现说明

- **v0.2.7 起默认只使用自建反代**：设置里的「API 反代地址」非空时，日历 / 详情 / 搜索**只请求它**，失败直接抛错并在界面提示（番剧表弹窗含「切换镜像站」按钮）——不会在用户不知情时换源。清空该字段即回到下面的镜像链。
- 反代与官方 v0 有两处差异：每日放送在 `/calendar`（不是 `/v0/calendar`），搜索必须是 `POST /v0/search/subjects`；详情必须是 `/v0/subjects/:id`（该反代在 `/subject/:id` 上返回的是**缺 infobox/tags 的旧版 API 形态**）。
- 公共镜像模式：镜像列表**并行尝试**（首个成功者生效）；API 镜像（`api.` 开头）自动走 v0 JSON 通道，网页镜像走 HTML 解析（bgm.tv 克隆站不提供 JSON API 且有 Cloudflare 防护，需浏览器 UA + HTML 解析，必要时改用离屏真实浏览器过校验）。
- 番剧表 30 分钟 / 详情与评分 7 天磁盘缓存；日历页本身不含评分，进入某天时按需补全（并发 2 + 间隔 120ms，避免把反代打成 503）。
- 图片经 `sakana-img://` 协议加载（磁盘缓存），官方图床（`lain.bgm.tv`）地址会按**原路径**改写到图片反代（保留 `/img` 前缀）。

## 文档

- `docs/项目说明书.md`：**逐文件夹、逐文件的详细项目说明书**（架构、播放子系统、规则引擎、数据存储、构建打包、`SAKANA_*` 自检开关全表、排障步骤、术语表）。
- `docs/代码审计.md`：全项目代码审计报告（高危/中危/轻微问题清单，均含 `文件:行号` 依据与建议修法）。

## 更新记录

> 这里只列最近两个版本；完整历史见本地文档 `docs/更新记录.md`（不入库）。

### 0.2.7

- **修「用反代很多番剧详情加载不出来」（本轮重点）**：标题、评分、上映日期都在，但**类型标签、制作信息、监督**整块空白 ——
  根因不是反代缺数据，而是应用请求错了路径：反代在 `/subject/:id` 上返回的是**旧版 API 形态**（约 2KB，只有
  `id/url/type/name/air_date/eps/rating`），天生没有 `infobox`/`tags`/`platform`。改为显式请求 `/v0/subjects/:id` 后，
  同一反代返回约 5KB 完整数据（实测《葬送的芙莉莲》：infobox 41 条、tags 30 个、平台 TV、总集数 36），
  详情页的「详细信息 / 类型标签 / 平台 / 总集数 / 导演 / 制作」全部回来了。
- **修详情页可能整页白屏**：v0 的 `infobox` 取值可能是 `[{v:"…"}]` 这种对象数组（如「别名」），
  直接渲染会让 React 抛错；现在统一压平为字符串。
- **「测试连接」不再把自建反代误报为失败**：反代日历在 `/calendar`，过去按 `/v0/calendar` 测（404）。
- **「当前数据源」显示与实际一致**：配了反代后仍显示 `bangumi.vip`（镜像列表第一项）的问题已修，
  并注明「镜像站当前不使用」（列表保留，清空反代即可切回）。
- **数据源默认只用自建反代**：`https://sankana-bangumi.de5.net/api`（图片 `…/img`）。
  **不再自动回退公共镜像** —— 反代不可用时在番剧表页弹窗说明原因，并给出「切换镜像站」按钮直接打开数据源配置；
  搜索页也会把失败原因留在页面上。适配了反代与官方 v0 的两处差异：每日放送在 `/calendar`、搜索必须 **POST** ——
  顺带修掉「API 源搜索一直 404」的老问题。
- **修封面图片加载不出来**：图片反代改写时用了 `origin`，把地址里的 `/img` 前缀丢掉了（404）；
  实测修正后 200 image/jpeg，应用内清空缓存重跑 22 秒成功缓存 9 张封面。
- **修「选集按钮点击无响应」**：悬浮窗是异步创建的，选集数据只推一次往往早于它就绪；现在会补推，
  数据未到时浮层显示「正在读取选集…」而不是空白。
- **修「看第 3 集却显示第 1 集 / 自动连播跳回第 2 集」**：进入播放页时没带当前线路与集序号，
  播放器按第 1 集计算，后续全错。现在规则页选集与继续观看都会带上，并按播放页地址兜底反查。
- 播放器内切集改为「按集数重跑一遍规则」在后台完成（与详情页选规则→选集数同一条流程），状态栏同步显示过程。
- **搜索页**：取消独立标记区域，书签改为**左侧可滑动列表**，点击书签在主区域看条目（可返回搜索），
  小窗口下同样是左侧竖列。

### 0.2.6

**播放器（本次重点）**
- 修复「自动连播总是跳到最后一集」：根因是切集后内核仍在上报**上一集**的 time/length/ended，
  新页面刚挂载就被判定为「已播完」，于是反复连播、一路快进。现在本集真正开播前一律忽略这些事件，
  并加了「时长 ≥60s、已播过 80%、开播超过 60s、两次连播间隔 ≥30s」多重闸门。
  实测：第 1 集播完 → 第 2 集，停在 `0:1`，不再跳最后一集。
- 播放器内切集改为**重新挂载播放页**（与「番剧详情 → 播放源 → 选集」完全同一条路），
  切换前先暂停当前播放；并修掉「切集时把播放页 HTML 当媒体交给内核」的老 bug。
- 切集补救链：直连 15s 未开播 → 重新嗅探拿新地址（对付短时效直链）→ 仍不行走 FFmpeg 中转 → 再失败才报错。
- 选集与详情面板改为**半透明浮层盖在画面上**（画在页面里会被原生视频窗口盖住，
  这正是「详情按钮点了没反应」的原因），打开浮层不再改动播放画面尺寸。
- 断点续播提示延长到 10 秒并移入悬浮窗（此前被视频盖住，只能看到一闪而过的提示）；
  观看进度改为**按集记录**，自动连播到下一集不会串用上一集的进度。
- 控制栏悬浮窗在主窗口失焦/最小化时自动隐藏（修「退到后台控制栏还浮在最前面」）。

**其他**
- 搜索页「标记区域」改造为**书签**：点击书签弹出条目列表、右上角添加书签、可重命名与删除；
  底部新增可定时切换的图片展示位（右键上传/删除展示图片）。
- 「错误日志」更名为「运行日志」，保留最近 1000 条（界面原先另有一处 500 条截断，已统一）。
- 关于页改用应用封面图标。

### 0.2.5

- **播放器全屏按钮修好了**：小窗口下控制栏也由透明悬浮窗绘制，而悬浮窗上的「全屏播放」按钮发的是
  `toggleFullscreen` 动作 —— 播放页**没有对应的处理分支**，点了完全没有反应（这就是「播放器无法全屏」的原因）。
  实测：`按钮动作 toggleFullscreen → 全屏=true`，再点一次 → `false`。
- **悬浮窗跟随窗口缩放**：主窗口恢复自由缩放后，悬浮窗不跟着变大小，控制栏按钮的实际位置与命中区域会错位
  （点了像没反应）。现在悬浮窗监听主窗口 resize/move/maximize/全屏事件实时同步边界
  （实测 主窗口 1144×763 → 悬浮窗 1145×764，跟随=true）。
- **窗口恢复自由缩放**：去掉 `will-resize` 拦截与 `maximizable:false`，最大化按钮回到标题栏；
  应用窗口与播放器全屏均正常（自检：`全屏=true 窗口=1536×864`，退出后回到 983×642）。
- **详情页只保留一个本地播放入口**：移除与「本地播放」重叠的「从文件夹播放」按钮
  （「本地播放」本来就会用所选文件所在文件夹建立播放列表）。
- **离屏取数窗口不再被误当成主窗口**：它同样是 BrowserWindow，会出现在 `getAllWindows()` 中，
  导致「第二个实例」可能把镜像站页面当主窗口弹出、自检模式取到错误窗口。现新增
  `realWindows()/focusedOrMain()` 统一排除，主窗口关闭时一并回收，并加了 5 分钟空闲自动关闭。

### 0.2.5

- **数据源切换到 `bangumi.vip`**（`bangumi.pro` 已被墙）：该站与 bangumi.pro 同款，
  `/calendar`、`/subject/<id>`、`/subject_search/<kw>?cat=2` 的结构完全一致，现有网页解析器直接复用。
  - **老安装自动迁移**：启动时把 `bangumi.vip` 补进镜像列表首位（只改默认值的话，已装用户永远拿不到新镜像）。
  - 自动剔除已确认不可达的 `bangumi.pro`（少发无用请求，实测能明显降低连接被重置的概率）。
- **新增离屏浏览器取数**（`services/offscreenFetch.ts`）：bangumi.vip 前置 Anubis 的 PoW 机器人校验，
  主进程 axios 只会拿到 14KB 的挑战页。现在网页镜像改为「axios 先试 → 拿到校验页就用离屏真实浏览器
  重新**导航**打开并读 DOM」，校验自动通过（应用内实测 1~14 秒）后拿到真正的服务端渲染 HTML。
  之所以用导航而不是页面内 `fetch`：实测同一站点导航已通过校验、紧接着的 fetch 仍可能被判未授权。
- **镜像竞速修正**：`requestBest` 过去用 `Promise.allSettled`，会被最慢的镜像拖住（有过校验要十几秒），
  现在改为**首个成功立即返回**；网络被重置（`ECONNRESET` 等）时自动隔 3 秒重试一次。
- 「测试连接」也走真实取数路径（含离屏浏览器），不再把 bangumi.vip 这类需要校验的站点误判为失败。
- 图片 Referer 规则补上 `lain.bangumi.vip`（实测无 Referer 也放行）。
- 自检工具：`scripts/diag-mirror-webview.cjs`（验证带机器人校验的镜像能否在真实浏览器里取到数据）、
  `SAKANA_OFFSCREEN_TEST=<url>`（在应用进程内验证离屏取数）。
- 实测：番剧表缓存刷新为 **7 天 / 111 条真实番剧**（图片来自 `lain.bangumi.vip`）；搜索页 32KB、条目详情页 129KB 均正常取得。
- AniBase 备用源的调研结论存档在 `docs/备用数据源调研.md`（其搜索接口对第三方封闭，暂不接入）。
