...
async function main() {
  ...
  const tok = token()
  const notes = [versionFile.notes ?? '', '', `下载安装包：${assetName}（${sizeMB} MB）`].join('\n')
  ...
  console.log(`开始上传 ${assetName}（${sizeMB} MB）→ uploads.github.com …`)
  const t0 = Date.now()
  const started = await uploadAsset(tok, release.upload_url.replace('{?name,label}', ''), assetPath, assetName)
  console.log(
